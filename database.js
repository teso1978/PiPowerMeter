...
var powerDb = new sqlite3.Database(databaseFile, function (err) {
    ...
    if (err) {
		...
		db.runSql("Insert or ignore into Config Values('VoltageScale', '380');", null, true);
                ...
                db.runSql("Insert or ignore into Config Values('Probes', '[{''Name'':''SCT-006'',''Factor'':''40''},{''Name'':''SCT-010'',''Factor'':''150''},{''Name'':''SCT-013'',''Factor'':''50''},{''Name'':''SCT-016'',''Factor'':''150''},{''Name'':''SCT-019'',''Factor'':''300''},{''Name'':''ZMCT103C'',''Factor'':''25''}]');", null, true);
...
var db =
{
  ...
  */
    ...
    updateProbe: function (id, type, board, currentChannel, voltageChannel, breaker, alert, probeType, callback) {

        var sql = '';

        if (id == null || id === undefined || id.toString() == '') {
            sql = "Insert into Probes Values(null,'" + type.escape() + "'," + board + ',' + currentChannel + ',' + voltageChannel + ',' + breaker + ",'" + alert + "'," + probeType + "); ";
        } else {
            sql = "Insert or replace into Probes Values(" + id + ",'" + type.escape() + "'," + board + ',' + currentChannel + ',' + voltageChannel + ',' + breaker + ",'" + alert + "'," + probeType + "); ";
        }
